...
def build_body() -> str:
    ...
    body.append(table([
        ...
        ["Presentation shell", "app/layout.tsx, components/app-shell.tsx, components/dashboard-nav.tsx", "Protected layout, sidebar navigation, role-specific menu visibility, notification badges, admin AI support bubble. The former floating calculator widget and the cloned per-table top scrollbar have both been removed; scrollbars are styled once in globals.css and tabbed pages share components/ui/tabs-nav.tsx."],
    ...
    body.append(table([
        ...
        ["Dashboard", "/dashboard", "Role-shaped summary: admins see business KPIs plus a weekly collections trend chart, an account status breakdown chart, and week-over-week up/down badges on KPI cards backed by createdAt/paymentDate history (lib/reports.ts getAdminDashboardTrend); staff see assigned operational metrics plus their own scoped weekly collections trend and today/this-week deltas (getStaffDashboardTrend). Cards with a real previous-period value render as an animated semicircular meter (components/reports/meter-gauge.tsx) whose progress arc sweeps from last week's position to this week's on load."],
