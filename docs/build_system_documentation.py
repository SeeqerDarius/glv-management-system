...
def build_body() -> str:
    ...
    body.append(para("The queue records unique event keys within the business transaction. Conditional claims prevent concurrent workers sending the same item. Before dispatch, source existence, account eligibility and current phone are checked. Missing phones and definitive failures are visible to super administrators at /settings/sms; correct the source then use Retry. Provider acceptance is labelled ACCEPTED, not confirmed handset delivery. Check BMS campaign history using the saved campaign ID. Rate limits retry after one hour, up to five attempts. Timeouts and interrupted workers become UNKNOWN and require reconciliation rather than blind resending. Restored pending or processing messages become UNKNOWN. Disabling SMS pauses the queue."))
