...
def build():
    ...
    doc.add_paragraph("Server setup requires MNOTIFY_API_KEY, MNOTIFY_SENDER_ID and CRON_SECRET, plus the SMS queue database migration. Store secrets only in the deployment environment. These alerts use a separate SMS queue; existing document and receipt channel preferences remain separate. Provider configuration and live delivery must be verified before describing the feature as operational.")
