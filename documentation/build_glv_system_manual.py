...
SECTIONS = [
    ...
    ("Procurement Overview", "08-procurement.png",
     "Identifies products that have crossed the configured collection threshold and may be ready for purchasing, and lets you confirm how many units you actually bought.",
     "Use it to prioritise procurement based on customer commitments and money already collected, then confirm the quantity procured so the list reflects what is still outstanding."),
    ("Procurement Details", "31-procurement-details.png",
     "Shows the customer accounts contributing to procurement demand for a selected product, plus the units already bought and awaiting delivery.",
     "Use it to validate quantities, collection progress, and the evidence behind a procurement decision, to confirm individual units, and to undo a confirmation entered in error."),
...
def build():
    ...
    for heading, body in [
        ...
        ("Confirm what you actually procured",
         "The procurement list shows the units that still need to be bought. After a buying trip, open the Procurement List tab, enter the number of units you actually bought for that product, and press Confirm procured. The list reduces by exactly that quantity, starting with the customers closest to finishing their plan, so a part purchase leaves the rest outstanding instead of clearing the whole product. If you bought a unit for one specific customer out of order, open the product from the procurement list and confirm that customer's unit on its own row. Confirming procurement is not the same as delivering: confirmed units move to Bought, awaiting delivery on the product page, and you still confirm delivery on the account when the customer receives the product. If you confirm the wrong quantity, use Undo on that page to put the unit back on the list."),
