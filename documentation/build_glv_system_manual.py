...
def build():
    ...
    doc.add_paragraph("Active or overdue plans with an outstanding balance receive a reminder after seven full days without payment, then at most once per further unpaid week. A newly entered payment, including a backdated one, restarts the reminder interval. Completed, closed, cancelled, archived, suspended, dormant and probation accounts do not receive weekly reminders. The Missed payment template supports staffName, which inserts the assigned staff member's first name. A daily job checks at 09:00 Ghana time; messages are also dispatched after qualifying actions and during signed-in use.")
    doc.add_paragraph("When a staff deposit is recorded for a week, GLV totals each assigned customer's payments from Monday through Sunday across all product accounts. Each customer who paid receives one weekly summary with that total and an encouragement to keep going. Customers with no payment receive no summary. The staff, customer and week key prevents duplicates if another deposit is recorded; a pending or failed summary may be refreshed, but an accepted summary is never resent automatically.")
