...
SECTIONS = [
    ...
    ("Procurement Details", "31-procurement-details.png",
     ...
     "Use it to validate quantities, collection progress, and the evidence behind a procurement decision. It is a read-only breakdown; confirm purchases from the Procurement List tab."),
...
def build():
    ...
    for heading, body in [
        ...
        ("Confirm what you actually procured",
         "The procurement list shows the units that still need to be bought. After a buying trip, open the Procurement List tab and press the procured button on the product's row. A small box asks how many units you bought. Enter the number and confirm, and the list reduces by exactly that quantity, so a part purchase leaves the rest outstanding instead of clearing the whole product. Confirming procurement is not the same as delivering: you still confirm delivery on the customer's account when they receive the product."),
