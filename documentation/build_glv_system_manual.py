...
SECTIONS = [
    ...
    ("Financial Intelligence and Reports", "10-reports.png",
     "Combines business performance, weekly staff collections, physical deposits, shortage or surplus, payroll, product profitability, and Excel export. The page opens with a section menu and groups the headline figures under Collections and Banking, Receivables and Exposure, Payroll, and Profitability.",
     "Use it for weekly control meetings, staff accountability, salary tracking, reconciliation, and management reporting. Jump straight to a section from the menu under the page title, and read each group together rather than card by card."),
...
def build():
    ...
    for heading, body in [
        ...
        ("Know what reactivation does to a closed account",
         "An administrator can bring back a dormant, probation or closed account once six months of inactivity have passed, using the amber Reactivate panel on the account page and confirming with an administrator password. GLV deducts a 32 percent service fee from what the customer had already paid, recalculates the balance, sends the customer a reactivation calculation, and voids any closure refund credit still open on that account, because the customer cannot keep the refund and the plan. A product already handed over stays delivered; an undelivered plan returns to pending. The account then restarts the dormancy ladder from the day it was reactivated, so it is not closed again by the next lifecycle check and is not charged the deduction twice. Collect a payment as soon as the plan restarts."),
        ("Retract a mistake within three hours",
         "Five actions can be taken back after they are done: reactivating an account, deleting a payment, deleting a staff deposit, deleting a salary payment, and marking a customer credit as refunded. For three hours afterwards an administrator sees the action listed with an Undo button, on the page where it was done and on the Activity page, with the time remaining shown. Retracting puts the affected records back exactly as they were. If something has changed in the meantime, such as a payment recorded against a reactivated account, GLV refuses the retraction and asks you to correct the record directly, so a retraction never quietly erases newer work. Messages and documents already sent to a customer are not withdrawn. After three hours the record stands and must be corrected through the normal workflow."),
    ...
    doc.add_paragraph("Active or overdue plans with an outstanding balance receive a missed-payment reminder after fourteen full days without payment, which is two full weeks, then at most once per further two-week period. A newly entered payment, including a backdated one, restarts the interval, and so does reactivating an account, so a reactivated plan is never texted about the months it spent dormant. Completed, closed, cancelled, archived, suspended, dormant and probation accounts do not receive missed-payment reminders. The Missed payment template supports staffName, which inserts the assigned staff member's first name. A daily job checks at 09:00 Ghana time; messages are also dispatched after qualifying actions and during signed-in use.")
