...
const sidebarCollapsedStorageKey = "glv-sidebar-collapsed";
...
export function AppShell({ children, user, brand }: {
  ...
  return (
    <div
      className={`glv-app-shell min-h-screen bg-gray-50 lg:grid lg:grid-cols-[var(--glv-sidebar-w)_minmax(0,1fr)] ${sidebarCollapsed ? "glv-sidebar-collapsed" : ""}`}
      style={{ "--glv-sidebar-w": sidebarCollapsed ? "5.25rem" : "17rem" } as React.CSSProperties}
    >
      ...
      <aside className={`glv-sidebar fixed inset-y-0 left-0 z-50 flex w-[17rem] flex-col text-white transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] lg:sticky lg:top-0 lg:h-screen lg:w-auto lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className={`flex h-18 shrink-0 items-center border-b border-white/10 px-5 ${sidebarCollapsed ? "lg:justify-center lg:px-0" : "justify-between"}`}>
          <div className={`flex min-w-0 items-center gap-3 ${sidebarCollapsed ? "lg:gap-0" : ""}`}>
            <span className="glv-brand-mark shrink-0">{tradingName.slice(0, 4)}</span>
            <div className={`glv-sidebar-fade min-w-0 ${sidebarCollapsed ? "glv-sidebar-fade-hidden" : ""}`}>
              <p className="truncate text-sm font-bold text-white">{companyName}</p>
              <p className="truncate text-xs text-lime-200">{tagline}</p>
            </div>
          </div>
          <button type="button" onClick={() => setMobileOpen(false)} className="inline-flex size-9 shrink-0 items-center justify-center rounded-md text-white/75 hover:bg-white/10 hover:text-white lg:hidden" aria-label="Close menu"><X className="size-5" /></button>
        </div>
        <DashboardNav
          isAdmin={isAdmin}
        ...
        />
        <div className={`mt-auto shrink-0 border-t border-white/10 p-4 ${sidebarCollapsed ? "lg:px-3" : ""}`}>
        ...
        <button
          ...
          className="glv-sidebar-toggle absolute -right-3 top-[4.5rem] z-10 hidden size-7 items-center justify-center rounded-full border border-white/10 text-white/70 shadow-lg hover:text-white lg:flex"
        >
          <span className={`flex transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${sidebarCollapsed ? "rotate-180" : "rotate-0"}`}>
