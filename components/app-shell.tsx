...
export function AppShell({ children, user, brand }: {
  ...
  return (
    <div className="glv-app-shell min-h-screen bg-gray-50 lg:grid lg:grid-cols-[16.5rem_minmax(0,1fr)]">
      ...
      <aside className={`glv-sidebar fixed inset-y-0 left-0 z-50 flex w-[16rem] max-w-[86vw] flex-col text-white transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:w-auto lg:max-w-none lg:translate-x-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex h-16 items-center justify-between border-b border-white/10 px-4 lg:h-18 lg:px-5">
      ...
      <div className="flex min-h-screen min-w-0 flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-2 border-b border-gray-200 bg-white/95 px-3 backdrop-blur sm:px-6 lg:h-18 lg:px-8">
          <div className="flex min-w-0 items-center gap-2 sm:gap-3">
            <button type="button" onClick={() => setMobileOpen(true)} className="inline-flex size-9 shrink-0 items-center justify-center rounded-md border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 lg:hidden" aria-label="Open menu"><Menu className="size-5" /></button>
            <div className="min-w-0"><p className="truncate text-base font-bold text-gray-950 sm:text-lg">{pageTitle}</p><p className="hidden text-xs text-gray-500 sm:block">GLV Management System</p></div>
          </div>
          <div className="min-w-[6rem] max-w-[42vw] text-right sm:max-w-none"><p className="truncate text-xs font-semibold text-gray-900 sm:text-sm">{user.name || "GLV User"}</p><div className="flex items-center justify-end gap-1 text-[0.68rem] text-gray-500 sm:gap-2 sm:text-xs">{user.staffCode ? <span className="font-semibold text-green-700">{user.staffCode}</span> : null}{user.staffCode ? <span aria-hidden="true">•</span> : null}<span className="truncate">{roleLabel}</span></div></div>
        </header>
        <main className="glv-main-content min-w-0 flex-1 px-3 py-4 sm:px-6 sm:py-7 lg:px-8 lg:py-8">
